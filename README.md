...
`MaxLenTokenizer.py` is an independent dynamic-programming encoder that minimizes the number of output tokens using only a list of model-visible tokens. `ConvertVocab.py` converts the `vocab.txt` file generated by any Pruned BPE trainer into the format required by this encoder.
